...
"""
🌙 ЛУНА v3.0 — СИСТЕМА НАПОМИНАНИЙ ПО СПЕЦИФИКАЦИИ

Таймиги: +3ч, +10ч, +20ч от начала молчания
Каждой категории — свои тексты, во 2м напоминании видео школы (молчуны/думающие) или демо (активные)
3е напоминание = опросник (вытащить реальное возражение)

По спецификации Камшат
Дата: 23 июля 2026
"""
...
MOLCHUN = "molchun"          # Написали, молчат (возражение неизвестно)
THINKING = "thinking"        # Была переписка, потом пропали
ACTIVE = "active"            # Ведётся диалог, потом прервалось
REFUSAL = "refusal"          # Отказались явно
AGGRESSION = "aggression"    # Послали нас

# ═══════════════════════════════════════════════════════════════════════════════
# ЦЕПОЧКИ НАПОМИНАНИЙ ПО КАТЕГОРИЯМ
# ═══════════════════════════════════════════════════════════════════════════════

REMINDER_CHAINS = {
    
    # 1. МОЛЧУНЫ — написали, но молчат (возражение неизвестно)
    # Цель: вытащить реальное возражение через опросник
    MOLCHUN: [
        (10800, (  # +3 часа
            "Иногда одно решение меняет гораздо больше, чем кажется… ✨\n"
            "Мы недавно обсуждали обучение 🙂\n"
            "Если закрутился день — я понимаю. Напишите когда удобно 🌿"
        )),
        (36000, (  # +10 часов
            "Посмотрите в какой школе вы будете обучаться 🎓\n"
            "Какой у нас уровень, обстановка, как проходит обучение:\n"
            "https://www.instagram.com/reel/DI3_-a3I4zA/?igsh=MmNzanl1Z2hmc2lr"
        )),
        (72000, (  # +20 часов
            "Давайте разбираться! 😊\n\n"
            "Что для вас главное при выборе школы?\n"
            "1️⃣ Цена\n"
            "2️⃣ Качество обучения\n"
            "3️⃣ Удобство по времени\n"
            "4️⃣ Наличие сертификата\n"
            "5️⃣ Поддержка лектора\n\n"
            "Ответьте цифрой 😊"
        )),
    ],
    
    # 2. ДУМАЮЩИЕ — была переписка, потом пропали
    # Цель: вернуть в диалог через видео школы
    THINKING: [
        (10800, (  # +3 часа
            "Понимаю что сложно выбирать 🤔\n"
            "Я помогу разобраться, не торопитесь.\n"
            "Напишите когда будут вопросы."
        )),
        (36000, (  # +10 часов
            "Посмотрите как обучается в нашей школе 🎓\n"
            "Какой уровень, какая атмосфера, как проходит практика:\n"
            "https://www.instagram.com/reel/DI3_-a3I4zA/?igsh=MmNzanl1Z2hmc2lr"
        )),
        (72000, (  # +20 часов
            ...
            "4️⃣ Наличие сертификата\n"
            "5️⃣ Поддержка лектора\n\n"
            "Ответьте цифрой — подберу идеально 💪"
        )),
    ],
    
    # 3. АКТИВНЫЕ — ведётся диалог, потом прервалось
    # Цель: убрать страхи, предложить конкретные шаги
    ACTIVE: [
        (10800, (  # +3 часа
            "Видно что вас интересует! 👍\n"
            "Классно когда готовы разбираться в деталях.\n"
            "Какие ещё вопросы? Я здесь."
        )),
        (36000, (  # +10 часов
            "Вы не переживайте! 🙂\n"
            "Есть возможность посмотреть демо-доступ,\n"
            "поговорить с лекторами, забронировать место.\n"
            "Если вас что-то беспокоит — обязательно спросите!"
        )),
        (72000, (  # +20 часов
            "Давайте на чистоту 😊\n\n"
            "Что дальше?\n"
            "1️⃣ Хочу посмотреть демо-доступ\n"
            "2️⃣ Нужна ясность с ценами\n"
            "3️⃣ Нужно время подумать\n"
            "4️⃣ Ищу отзывы о школе\n"
            "5️⃣ Готов(а) оплатить\n\n"
            "Цифра — помогу! 🚀"
        )),
    ],
    
    # 4. ОТКАЗАВШИЕСЯ — максимум одно через неделю
    REFUSAL: [
        (604800, (  # 7 дней
            "Если вдруг передумали — я здесь 🙂\n"
            "Без давления, просто дверь открыта."
        )),
    ],
    
    # 5. АГРЕССИВНЫЕ — не отправляем вообще
    AGGRESSION: [],
}

# Совместимость со старым кодом — базовая цепочка для молчунов
REMINDER_CHAIN = REMINDER_CHAINS[MOLCHUN]

# ═══════════════════════════════════════════════════════════════════════════════
# ФУНКЦИЯ КЛАССИФИКАЦИИ (DeepSeek)
# ═══════════════════════════════════════════════════════════════════════════════

async def classify_client_type_deepseek(chat_id: str, history: list) -> str:
    """
    Анализирует диалог через DeepSeek и определяет категорию.
    
    MOLCHUN: написали, молчат (диалога почти не было)
    THINKING: была переписка, потом пропали (несколько сообщений туда-сюда)
    ACTIVE: ведётся активный диалог, потом прервалось (много вопросов)
    REFUSAL: явно отказались ("нет", "отказываюсь")
    AGGRESSION: грубо ругаются ("нахуй", "послать")
    """
    
    if not history or len(history) < 2:
        return MOLCHUN
    
    # Собираем последние 8 сообщений для анализа
    user_messages = [msg for msg in history if msg.get("role") == "user"]
    if not user_messages:
        return MOLCHUN
    
    last_messages = user_messages[-8:]
    dialogue_text = "\n".join([msg.get("content", "") for msg in last_messages])
    
    classify_prompt = f"""
Проанализируй диалог клиента и определи категорию. ОТВЕТЬ ТОЛЬКО ОДНО СЛОВО.

Диалог:
{dialogue_text}

Категории:
- MOLCHUN: написал(а) 1-2 сообщения, больше не отвечает, диалога почти не было
- THINKING: была какая-то переписка (3+ сообщений туда-сюда), потом пропал(а)
- ACTIVE: активно задает вопросы, много переписки, потом вдруг прервалось
- REFUSAL: явно сказал(а) "нет", "отказываюсь", "не интересует", "не подходит"
- AGGRESSION: грубо ругается, посылает нас, агрессивный тон

Ответь ОД НО СЛОВОМ (больше ничего):
MOLCHUN или THINKING или ACTIVE или REFUSAL или AGGRESSION
"""
    ...
    try:
        ...
        # Валидация
        valid = {MOLCHUN, THINKING, ACTIVE, REFUSAL, AGGRESSION}
        if result in valid:
            log.info(f"🏷️ {chat_id}: {result}")
            return result
        else:
            log.warning(f"⚠️ {chat_id}: неизвестная категория '{result}', используем MOLCHUN")
            return MOLCHUN
            
    except Exception as e:
        log.error(f"❌ {chat_id}: ошибка классификации: {e}")
        return MOLCHUN


# ═══════════════════════════════════════════════════════════════════════════════
# ОТПРАВКА НАПОМИНАНИЯ С УЧЁТОМ КАТЕГОРИИ
# ═══════════════════════════════════════════════════════════════════════════════

async def send_reminder_chain_step(chat_id: str, step: int) -> None:
    """
    Отправляет напоминание из цепочки нужной категории.
    Таймиги: 3ч, 10ч, 20ч
    """
    try:
        async with db_pool.acquire() as conn:
            current_step = await get_reminder_step(chat_id)
            if current_step is None or current_step != step:
                log.debug(f"🔕 {chat_id}: пропускаем шаг {step}")
                return
            
            # Получаем состояние и историю
            state, history, _, _ = await get_state(chat_id)
            if state is None:
                log.info(f"🔕 {chat_id}: нет диалога")
                return
            
            # Не отправляем если есть активный диалог (< 1 часа)
            last_user_msg_time = None
            if history:
                for msg in reversed(history):
                    if msg.get("role") == "user":
                        last_user_msg_time = datetime.now(UTC) - timedelta(hours=1)
                        break
            
            if last_user_msg_time and (datetime.now(UTC) - last_user_msg_time) < timedelta(hours=1):
                log.info(f"💬 {chat_id}: активный диалог — не напоминаем")
                return
            
            # Не отправляем если менеджер взял чат
            if state == STATE_MANAGER:
                log.info(f"👨‍💼 {chat_id}: менеджер взял — не напоминаем")
                return
            
            # ГЛАВНОЕ: определяем категорию через DeepSeek
            client_type = await classify_client_type_deepseek(chat_id, history)
            
            # Выбираем цепочку
            reminder_chain = REMINDER_CHAINS.get(client_type, REMINDER_CHAINS[MOLCHUN])
            
            # Если цепочка пуста (AGGRESSION) — не отправляем
            if not reminder_chain:
                log.info(f"🚫 {chat_id}: {client_type} — не отправляем")
                await cancel_reminder_chain(chat_id)
                await save_reminder_step(chat_id, None)
                return
            
            # Проверяем границы цепочки
            if step >= len(reminder_chain):
                log.info(f"✅ {chat_id}: цепочка завершена")
                await cancel_reminder_chain(chat_id)
                await save_reminder_step(chat_id, None)
                return
            
            delay_sec, text = reminder_chain[step]
            
            # Отправляем
            await send_to_chat(chat_id, text)
            log.info(f"📬 {chat_id}: напоминание {step+1}/3 ({client_type})")
            
            # Планируем следующий шаг
            if step + 1 < len(reminder_chain):
                schedule_reminder_chain(chat_id, step + 1)
                await save_reminder_step(chat_id, step + 1)
            else:
                log.info(f"✅ {chat_id}: цепочка завершена")
                await cancel_reminder_chain(chat_id)
                await save_reminder_step(chat_id, None)
        ...
        log.error(f"❌ send_reminder_chain_step({chat_id}, {step}): {e}", exc_info=True)


# ═══════════════════════════════════════════════════════════════════════════════
# ОБРАБОТКА ОТВЕТОВ НА ОПРОСНИКИ
# ═══════════════════════════════════════════════════════════════════════════════

async def handle_reminder_oprosnik(chat_id: str, text: str, history: list) -> str:
    """
    Обрабатывает ответ клиента на опросник (3е напоминание).
    По ответу даёт адресный ответ Луны.
    """
    
    client_type = await classify_client_type_deepseek(chat_id, history)
    
    if text.strip() not in ["1", "2", "3", "4", "5"]:
        return None  # Не опросник, пропускаем
    
    choice = text.strip()
    
    # МОЛЧУНЫ — "Что для вас главное?"
    if client_type == MOLCHUN:
        responses = {
            "1": "Понимаю! 💡 Рассрочка Kaspi 0-0-12: это всего ~5-8 тыс/месяц, без переплат. За первый месяц работы окупается полностью. Вот ссылка оплаты: https://pay.kaspi.kz/pay/3o17lr5z",
            "2": "Отлично! Качество — это наше главное. Единственная школа в Казахстане с уклоном в ЛФК и медицинский фитнес. Лекторы с высшим медобразованием, 15+ лет опыта. Результат — 2000+ выпускников 💪",
            "3": "Идеально! Онлайн: 24/7 доступ, учитесь в своём темпе. Офлайн: гибкое расписание (выходные или будни). Вы сами выбираете что удобнее 😊",
            "4": "Да, это важно! Сертификат ISO 9001 (международный). Свидетельство о профквалификации. Достаточно для трудоустройства или открытия своего бизнеса 📄",
            "5": "Большой плюс! Тариф ЭКСПЕРТ — связь в Telegram, Zoom-встречи 4 раза/месяц, разбор техники. Вы не один, мы рядом на каждом шагу 🤝",
        }
    
    # ДУМАЮЩИЕ — "Что главное при выборе?"
    elif client_type == THINKING:
        responses = {
            "1": "Рассрочка Kaspi 0-0-12: всего ~5-8 тыс/месяц. Первый месяц работы окупает полностью. Первый платёж: https://pay.kaspi.kz/pay/3o17lr5z 💳",
            "2": "Единственная школа в Казахстане с уклоном в ЛФК и медицинский фитнес! Лекторы только с высшим медобразованием. 15+ лет опыта каждый. 2000+ выпускников 🏆",
            "3": "Гибкое расписание! Онлайн 24/7. Офлайн: выходные (сб-вс) или будни (вт-чт), по 4-5 часов. Вы выбираете что удобнее 📅",
            "4": "Всем выдаём сертификат ISO 9001. Офлайн студенты дополнительно получают свидетельство о профквалификации. Оба достаточны для трудоустройства или своего бизнеса 📜",
            "5": "Лекторы 24/7 в Telegram! Zoom-встречи каждую неделю, разбор вашей техники, ответы на все вопросы. Вы не одни 🤝",
        }
    
    # АКТИВНЫЕ — "Что дальше?"
    elif client_type == ACTIVE:
        responses = {
            "1": "Вот две ссылки:\n\nФитнес-тренер: https://champion-school.skillspace.ru/l/fitness-instruktor-s-nula\n\nЙога и растяжка: https://champion-school.skillspace.ru/l/yoga-stretching-oda\n\nСтеп и функциональный: https://champion-school.skillspace.ru/l/step-aerobics-functional-training\n\nЗайдите → выберите 'Демо доступ' → введите ФИ. Я активирую доступ! 🎯",
            ...
            "5": "Отлично! 🎉 Вот ссылка оплаты: https://pay.kaspi.kz/pay/3o17lr5z\n\nКасpi, Halyk, обычная оплата, рассрочка доступна. Я помогу с оформлением. Добро пожаловать в команду! 💪",
        }
    
    else:
        return None
    
    return responses.get(choice, "Спасибо за ответ! 😊")
